...
{% if request.user|can_hijack:another_user %}
  <form action="{% url 'hijack:acquire' %}" method="POST">
   {% csrf_token %}
   <input name="user_pk" type="hidden" value="{{ another_user.pk }}"/>
   <button type="submit">
    impersonate {{ another_user }}
   </button>
...
A form is used to perform a POST including a [CSRF]-token for security reasons.
...
If not provided, users are forwarded to the [LOGIN_REDIRECT_URL].
