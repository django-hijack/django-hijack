...
def check_url_allowed_attributes(app_configs, **kwargs):
    ...
    if not any(
        attribute in set_attributes for attribute in required_attributes
    ):
        error_msg = (
            'Setting HIJACK_URL_ALLOWED_ATTRIBUTES needs to be subset of ({})')
        errors.append(
            Error(
                error_msg.format(', '.join(required_attributes)),
                hint=None,
                obj=set_attributes,
                id='hijack.E001',
            )
