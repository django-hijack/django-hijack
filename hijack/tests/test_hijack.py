...
class HijackTests(BaseHijackTests):
    ...
    def test_basic_hijack(self):
        ...
        hijacked_response = client.post(
            '/hijack/%d/' % self.user.id, follow=True)
        self.assertEqual(hijacked_response.status_code, 200)
        hijack_released_response = client.post(
            '/hijack/release-hijack/', follow=True)
    ...
    def assertHijackPermissionDenied(self, response):
        ...
        self.assertFalse(
            getattr(self.client.session, 'is_hijacked_user', False))
        self.assertFalse('hijacked-warning' in str(response.content))
    ...
    def test_hijack_urls(self):
        ...
        self.assertEqual(
            '/hijack/release-hijack/', reverse('hijack:release_hijack'))
        self.assertEqual(
            '/hijack/1/', reverse('hijack:login_with_id', args=[1]))
        self.assertEqual(
            '/hijack/2/',
            reverse('hijack:login_with_id', kwargs={'user_id': 2})
        )
        self.assertEqual(
            '/hijack/username/bob/',
        ...
        self.assertEqual(
            ...
            reverse(
                'hijack:login_with_username', kwargs={'username': 'bob_too'}),
        )
        self.assertEqual(
    ...
    def test_hijack_url_user_id(self):
        ...
        response = self.client.post(
            '/hijack/%s/' % self.user.username, follow=True)
    ...
    def test_hijack_url_email(self):
        response = self.client.post(
            '/hijack/email/%s/' % self.user_email, follow=True)
        self.assertHijackSuccess(response)
        self._release_hijack()
        response = self.client.post(
            '/hijack/email/dfjak@hdl.com/', follow=True)
        self.assertEqual(response.status_code, 404)

    def test_hijack_permission_denied(self):
        self.client.logout()
        self.client.login(
            username=self.staff_user_username,
    ...
    def test_last_login_time_not_changed(self):
        ...
        self._release_hijack()
        self.assertEqual(
            User.objects.get(id=self.user.id).last_login, user_last_login)
        self.assertEqual(
    ...
    def test_settings(self):
        ...
        self.assertTrue(
            hasattr(hijack_settings, 'HIJACK_URL_ALLOWED_ATTRIBUTES'))
        self.assertEqual(
    ...
    def test_bootstrap_option(self):
        with SettingsOverride(hijack_settings, HIJACK_USE_BOOTSTRAP=True):
            ...
            response = self.client.get('/hello/')
            self.assertTrue(
                'hijacked-warning-bootstrap' in str(response.content))
            response = self.client.get('/hello/filter/')
            self.assertTrue(
                'hijacked-warning-bootstrap' in str(response.content))
    ...
    def test_signals(self):
        ...
        def hijack_started_receiver(
            ...
            **kwargs
            ...
            received_signals.append(
                'hijack_started_%d_%d' % (hijacker_id, hijacked_id))
        ...
        def hijack_ended_receiver(
            ...
            **kwargs
            ...
            self.assertEqual(hijacked_id, hijacked.id)
            received_signals.append(
                'hijack_ended_%d_%d' % (hijacker_id, hijacked_id))
        ...
        self.assertIn(
            'hijack_ended_%d_%d' % (self.superuser.id, self.user.id),
            received_signals,
        )
