# Test cases for settings-dependent urls/decorators.
# Need to be run separately from HijackTests.
...
class AllowGetRequestsHijackTestCase(BaseHijackTests):
    ...
    @skipIf(
        ...
        (
            'Skipping test cases for settings-dependent urls/decorators '
...
class CustomDecoratorHijackTestCase(BaseHijackTests):
    ...
    def test_custom_decorator(self):
        ...
        with SettingsOverride(
            hijack_settings, HIJACK_DECORATOR=custom_decorator_path
        ):
            self.assertEqual(
                hijack_settings.HIJACK_DECORATOR, custom_decorator_path)
            self.client.logout()
