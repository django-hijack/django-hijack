...
class ChecksTests(TestCase):
        ...
        with SettingsOverride(settings, ALLOW_STAFF_TO_HIJACKUSER=True):
            ...
            expected_warnings = [
                Warning(
                    (
                        'Deprecation warning: '
                        'Setting "ALLOW_STAFF_TO_HIJACKUSER" '
                        'has been renamed to "HIJACK_AUTHORIZE_STAFF"'
                    ),
                    hint=None,
                    obj=None,
                    id='hijack.W002'
    ...
    def test_check_url_allowed_attributes(self):
        ...
        with SettingsOverride(
            hijack_settings,
            HIJACK_URL_ALLOWED_ATTRIBUTES=('username', 'email'),
        ):
            errors = checks.check_url_allowed_attributes(HijackConfig)
            self.assertFalse(errors)
    ...
    def test_hijack_decorator_importable(self):
        ...
        self.assertFalse(errors)
        with SettingsOverride(
            hijack_settings, HIJACK_DECORATOR='not.importable'
        ):
            expected_errors = [
                Error(
    ...
    def test_check_staff_authorization_settings(self):
        ...
        with SettingsOverride(
            ...
            expected_errors = [
                Error(
                    (
                        'Setting HIJACK_AUTHORIZE_STAFF_TO_HIJACK_STAFF '
                        'may not be True if HIJACK_AUTHORIZE_STAFF is False.'
                    ),
                    hint=None,
                    obj=None,
                    id='hijack.E004',
